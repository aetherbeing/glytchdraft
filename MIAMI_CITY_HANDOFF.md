...
| Stage | What it does |
|---|---|
| `extract` | PDAL HAG filter — separates building candidate points from ground |
| `clean` | Statistical outlier removal on building points |
| `cluster` | DBSCAN (eps=3.0m, min_samples=10) — isolates individual buildings |
| `footprints` | Convex hull + rotated bbox polygons per cluster |
| `masses` | Height estimation (p90) + OBJ extrusion (LOD0 + LOD1) |
| `manifest` | Writes per-tile `{tile_id}_manifest.json` |
...
```
scripts/miami/miami_city_config.py      Main config — paths, CRS, DBSCAN params, address source
scripts/miami/run_miami_city.py         City orchestrator — Live dashboard, address ingest, manifest
scripts/miami/run_tile_miami.py         Per-tile subprocess — all 5 stages self-contained
...
1. Run `--execute` on all 108 tiles (~3–5 hours estimated at ~2.7 min/tile)
...
5. See `PIPELINE_REFACTOR.md` for the 4-phase plan to make LA/NYC pipelines match Miami's address enrichment + audit contract
