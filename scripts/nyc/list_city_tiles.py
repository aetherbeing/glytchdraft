"""
list_city_tiles.py  [NYC city pipeline - GlitchOS.io]

Show all NOAA Coastal LiDAR tiles required to process a city boundary, their local
LAZ availability, estimated download size, and output paths.

Usage:
    python scripts/nyc/list_city_tiles.py --city new_york_city
    python scripts/nyc/list_city_tiles.py --city new_york_city --refresh
    python scripts/nyc/list_city_tiles.py --city new_york_city --limit 20
    python scripts/nyc/list_city_tiles.py --city new_york_city --json
"""
...
def _print_tile_table(tiles: list[TileInfo], city_id: str):
    ...
    tbl = Table(
        ...
        show_lines=False,
        title=f"[bold]{cfg.display_name} - NOAA Tile Manifest[/bold]",
    )
    ...
    if n_missing == 0:
        ...
        console.print(
            f"\nReady to execute:\n"
            f"  [cyan]python scripts/nyc/run_city.py {city_id} --execute[/cyan]"
        )
    else:
        console.print(
    ...
    console.print(
        f"  [cyan]python scripts/nyc/list_city_tiles.py --city {city_id} --refresh[/cyan]"
        "   — re-run tile discovery"
    )
    console.print(
        f"  [cyan]python scripts/nyc/run_city.py {city_id} --dry-run[/cyan]"
