"""
stages/s05_manifest.py  [NYC city pipeline]
...
from tile_config import TileConfig, BLOCK_MANIFEST_PATH, TILE_ORDER, TILES, SRC_EPSG, DST_EPSG
...
def write_tile_manifest(tile: TileConfig, stage_results: dict) -> Path:
    ...
    manifest = {
        ...
        "pipeline":         "GlitchOS.io NYC tile pipeline",
...
def write_block_manifest(tile_results: dict[str, dict]) -> Path:
    ...
    block_manifest = {
        "schema_version":   PIPELINE_VERSION,
        "pipeline":         "GlitchOS.io NYC block pipeline",
        "block_id":         "nyc_dynamic",
        "description":      "Dynamic NYC tile block",
        "source_crs":       f"EPSG:{SRC_EPSG}",
        "target_crs":       f"EPSG:{DST_EPSG}",
