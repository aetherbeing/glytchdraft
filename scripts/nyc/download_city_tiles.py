"""
download_city_tiles.py  [NYC city pipeline - GlitchOS.io]

Download missing LAZ tiles listed in the city tile manifest.

Reads:
  /mnt/t7/nyc/data_processed/cities/<city_id>/tile_manifest.json

Downloads to:
  /mnt/t7/nyc/data_raw/laz/

Skips files that are already present on disk.
Shows per-file byte progress and overall tile count progress.

Usage:
    python scripts/nyc/download_city_tiles.py
    python scripts/nyc/download_city_tiles.py --city new_york_city
    python scripts/nyc/download_city_tiles.py --dry-run
    python scripts/nyc/download_city_tiles.py --limit 3
    python scripts/nyc/download_city_tiles.py --workers 2
...
# ── NOAA download URL pattern for NYC 2017 topobathy.
# Tried in order until one returns HTTP 200.

NOAA_URL_TEMPLATES = [
    "https://noaa-nos-coastal-lidar-pds.s3.amazonaws.com/laz/geoid18/9306/{filename}",
]
...
def _load_manifest(city_id: str) -> list[dict]:
    ...
    if not cfg.tile_manifest.exists():
        ...
        console.print(
            f"Run first:\n"
            f"  [cyan]python scripts/nyc/list_city_tiles.py --city {city_id}[/cyan]"
        )
...
def _resolve_url(tile: dict) -> str | None:
    ...
    Prefers the URL stored in the manifest, then tries the known NOAA
    Coastal LiDAR S3 URL template with a HEAD request.
    """
    ...
    for template in NOAA_URL_TEMPLATES:
...
def dry_run(city_id: str, limit: int | None):
    ...
    for t in missing:
        ...
        src    = "[dim]manifest URL[/dim]" if stored else "[yellow]NOAA template[/yellow]"
    ...
    console.print(
        f"\nTo execute:\n"
        f"  [cyan]python scripts/nyc/download_city_tiles.py --city {city_id}[/cyan]"
    )
    return 0
...
def download(city_id: str, limit: int | None, workers: int):
    ...
    if still_missing == 0:
        ...
        console.print(f"  [cyan]python scripts/nyc/run_city.py {city_id} --execute[/cyan]")
...
def main():
    ...
    try:
        ...
        console.print("[red]Cannot import city_config. Run from the scripts/nyc/ directory "
