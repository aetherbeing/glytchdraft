"""
stages/s04_masses.py  [LA block pipeline]
...
Z UNIT NOTE: LAZ Z is in US survey feet (EPSG:2229 compound CRS).
PDAL reprojection to EPSG:32611 (2D) passes Z through unchanged.
All Z arithmetic is done after multiplying by FTUS_TO_M.
...
from tile_config import TileConfig, SRC_SRS, DST_SRS, FTUS_TO_M
...
def _write_obj(path, polys, stats, include, use_obb, tile_id, header_extra=""):
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write(f"# GlitchOS.io — LA block {tile_id} building masses\n")
        f.write(f"# CRS: EPSG:32611 (UTM 11N, meters). NO Blender shift applied.\n")
        if header_extra:
...
def _write_metadata(path, polys, attrs, stats):
    ...
    path.write_text(json.dumps({
        ...
        "name": "glitchos_la_building_masses",
