...
def _print_tile_table(tiles: list[TileInfo], city_id: str):
    ...
    tbl.add_column("Download URL", style="dim",    min_width=12)

    for t in tiles:
        disk_str = "[green]✓[/green]" if t.on_disk else "[red]✗ missing[/red]"
        size_str = f"{t.file_size_mb:.0f} MB" if t.file_size_mb else "—"
        url_str  = "[dim]via API[/dim]" if t.download_url else "[dim]grid[/dim]"
        tbl.add_row(
            t.tile_id,
            t.laz_filename,
            disk_str,
    ...
    if n_missing == 0:
        ...
        console.print(f"[yellow]{n_missing} LAZ tile(s) missing — "
    ...
    console.print(f"  [cyan]python scripts/la/list_city_tiles.py --city {city_id} --refresh[/cyan]"
                  "   — re-run tile discovery")
    console.print(f"  [cyan]python scripts/la/run_city.py {city_id} --dry-run[/cyan]"
...
def main():
    ...
    if not refresh:
        ...
        if tiles:
            cfg = CITIES[city_id]
            console.print(f"[dim]Using cached manifest ({len(tiles)} tiles): {cfg.tile_manifest}[/dim]")
