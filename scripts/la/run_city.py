...
def execute(city_id: str, stages: list[str]):
    ...
    # Load tile manifest (require list_city_tiles.py to have run first)
    if not cfg.tile_manifest.exists():
        console.print("[red]Tile manifest not found. Run first:[/red]")
    ...
    try:
        ...
        console.print(f"[red]Failed to read tile manifest: {e}[/red]")
        return 1
    ...
    n_fail = len(runnable) - n_ok

    console.print()
    console.print(Panel(
        "\n".join([
            f"[bold]City [cyan]{city_id}[/cyan] complete[/bold]",
            f"  {n_ok}/{len(runnable)} tiles OK   "
